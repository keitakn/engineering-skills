---
name: claude-plan-review-loop
description: Claude Code CLIで実装計画レビューの改善ループを回す。実装計画ファイルと要件ファイルを指定し、Claude Codeにレビューさせ、指摘をトリアージして計画を修正し、再レビューする。
---

# 実装計画レビューループ（レビュアー: Claude Code CLI）

Claude Code CLI（`claude -p`）をレビュアーとして、実装計画の「指摘 → 修正」の改善ループを回します。AIが作成した実装計画を、実装に入る前に独立したレビュアーで検証するためのスキルです。

プロンプト生成・Claude Code実行・結果解析・waiver適用・停滞検出・エビデンス検証は、すべて同梱スクリプトが行います。あなた（このスキルを実行するエージェント）の担当は次のとおりです。
...
スクリプト: このSKILL.mdがあるディレクトリ配下の `scripts/plan_review.py`（例: `~/.codex/skills/claude-plan-review-loop/scripts/plan_review.py`。python3 標準ライブラリのみで動作）

## 前提条件

- `claude` コマンドがインストール済み・認証済み
- レビュー対象プロジェクトの作業ディレクトリで実行する（セッション再開が同一ディレクトリに限定されるため、レビューループの途中で作業ディレクトリを変えない）

## 手順

### 1. 対象ファイルの確認

- ユーザーの指示から planFile（レビュー対象の実装計画ファイル）と promptFile（実装計画の元となった要件ファイル）を特定する
- 不足している場合は、ユーザーに1つずつ確認する

### 2. 開始

```bash
python3 <スキルディレクトリ>/scripts/plan_review.py start --backend claude --plan "<planFile>" --req "<promptFile>"
```
...
```bash
python3 <スキルディレクトリ>/scripts/plan_review.py review --run "<runDir>"
```

2回目以降(計画修正後)は、修正サマリーのファイルを渡す。

```bash
python3 <スキルディレクトリ>/scripts/plan_review.py review --run "<runDir>" --changes-file "<runDir>/changes-round<N>.md"
```
...
```bash
python3 <スキルディレクトリ>/scripts/plan_review.py feedback --run "<runDir>" --file "<runDir>/feedback.txt"
```
...
1. 各 blocking finding の指摘内容に基づき、planFile を直接修正する（promptFile の要件と矛盾しないこと。レビュアー側のClaude Codeにはファイルを編集させない設定になっている）
...
- プロジェクト単位の永続設定: リポジトリルートの `.agents/plan-review.json`（存在すれば自動で読み込む）
  - 使えるキーと既定値はスクリプト冒頭の `DEFAULT_CONFIG` を参照（maxRounds, blockingCategories, blockingSeverities, perspectives, additionalInstructions, claudeExtraArgs など）
...
| 症状 | 対応 |
| --- | --- |
| `claude` が見つからない | Claude Codeの公式インストール手順の確認をユーザーに案内する |
| 認証エラー（401等） | `claude auth status` で状態を確認し、必要なら `claude auth login`（それでも直らない場合は `claude auth logout` 後に再ログイン）を案内する |
| `is_error: true` で失敗 | スクリプトが result と permission_denials を表示する。内容をそのままユーザーに報告する |
